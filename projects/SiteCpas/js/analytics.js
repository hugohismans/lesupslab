...
// ── Top agents (restreint chefs+) ──────────────────────────────────
ANALYTICS.register({
  id: 'top-agents', label: 'Agents les plus actifs', size: 'medium', category: 'reservations',
  render(el, { from, to }) {
    if (!DB.hasPermission('managePlanning')) {
      el.innerHTML = '<p class="an-empty">🔒 Réservé aux chefs de service et à la direction.</p>';
      return;
    }
    const counts = {};
    _anOccs(from, to).forEach(r => {
...
// ═══════════════════════════════════════════════════════════════════
// Widget Présences du jour — restreint chefs de service / direction
// ═══════════════════════════════════════════════════════════════════
ANALYTICS.register({
  id: 'presence-log',
  ...
  render(el) {
    ...
    const today = isoDate(new Date());
    el.innerHTML = `
      <div class="an-pl-header">
        <label class="an-pl-date-label">Journée du
          <input type="date" class="an-pl-date" value="${today}" max="${today}">
        </label>
        <span class="an-pl-legend">
          <span class="an-pl-badge an-pl-ok">✓ À l'heure</span>
          <span class="an-pl-badge an-pl-late">⚠ Retard</span>
          <span class="an-pl-badge an-pl-absent">Absent·e</span>
        </span>
      </div>
      <div class="an-pl-table-wrap" id="anPlTable${Date.now()}">
        <p class="an-empty">Chargement…</p>
      </div>`;

    const dateInput = el.querySelector('.an-pl-date');
    const tableWrap = el.querySelector('.an-pl-table-wrap');
    ...
    async function loadDay(date) {
      tableWrap.innerHTML = '<p class="an-empty">Chargement…</p>';
      let log = {};
      try { log = await DB.fetchPresenceLog(date); } catch(_) {}

      const agents = DB.getAgentsWithKeys().sort((a, b) => a.name.localeCompare(b.name, 'fr'));
      const isToday = date === today;

      const rows = agents.map(({ key, name }) => {
        const entry       = log[key] || {};
        const connectedAt = entry.connectedAt || null;
        const logoutAt    = entry.logoutAt    || null;
        ...
        let badge;
        if (!connectedAt) {
          badge = `<span class="an-pl-badge an-pl-absent">${isToday ? 'Pas encore là' : 'Absent·e'}</span>`;
        } else if (isLate) {
          const declared = entry.arrivalTime ? ` — annoncé ${entry.arrivalTime}` : '';
          badge = `<span class="an-pl-badge an-pl-late">⚠ Retard${declared}</span>`;
        } else {
          badge = `<span class="an-pl-badge an-pl-ok">✓ À l'heure</span>`;
        }
        ...
        if (!connectedAt) {
          ...
          logoutCell = fmt(logoutAt);
        } else if (isDone) {
          logoutCell = '<span class="an-pl-gone">Parti·e</span>';
        } else {
          logoutCell = isToday ? '<span class="an-pl-still">Présent·e</span>' : '—';
        }

        return `<tr>
          <td class="an-pl-name">${escapeHtml(name)}</td>
          <td class="an-pl-time">${connectedAt ? fmt(connectedAt) : '—'}</td>
          <td class="an-pl-time">${logoutCell}</td>
          <td>${badge}</td>
        </tr>`;
      });

      const present  = agents.filter(({ key }) => log[key]?.connectedAt).length;
      const absent   = agents.length - present;
      const late     = agents.filter(({ key }) => log[key]?.status === 'late').length;

      tableWrap.innerHTML = `
        <div class="an-pl-summary">
          <span>👥 <strong>${present}</strong> présent·e·s</span>
          <span>⚠ <strong>${late}</strong> en retard</span>
          <span>❌ <strong>${absent}</strong> absent·e·s</span>
        </div>
        <table class="an-pl-table">
          <thead>
            <tr>
              <th>Agent</th>
              <th>🟢 Bonjour</th>
